...
export async function migrate(pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (userRows[0].count === 0) {
      const hashedPassword = bcrypt.hashSync('gestion2030', 10);
      await conn.execute(
        'INSERT IGNORE INTO users (id, username, email, password, full_name, role) VALUES (?, ?, ?, ?, ?, ?)',
        ['u-admin', 'gestionpro', 'admin@elkaram.dz', hashedPassword, 'Administrateur', 'admin']
      );
    }
    // Update admin credentials on every start
    const hashedAdminPw = bcrypt.hashSync('gestion2030', 10);
