...
export async function migrate(pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (userRows[0].count === 0) {
      const hashedPassword = bcrypt.hashSync('gestion2630', 10);
      await conn.execute(
        'INSERT IGNORE INTO users (id, username, email, password, full_name, role) VALUES (?, ?, ?, ?, ?, ?)',
        ['u-admin', 'gestion26', 'admin@elkaram.dz', hashedPassword, 'Administrateur', 'admin']
      );
    }
    // Update admin credentials on every start
    const hashedAdminPw = bcrypt.hashSync('gestion2630', 10);
