...
async function migrate(pool) {
    ...
    `);
        ...
        if (userRows[0].count === 0) {
            const hashedPassword = bcryptjs_1.default.hashSync('gestion2030', 10);
            await conn.execute('INSERT IGNORE INTO users (id, username, email, password, full_name, role) VALUES (?, ?, ?, ?, ?, ?)', ['u-admin', 'gestionpro', 'admin@elkaram.dz', hashedPassword, 'Administrateur', 'admin']);
        }
        // Update admin credentials on every start
        const hashedAdminPw = bcryptjs_1.default.hashSync('gestion2030', 10);
        await conn.execute("UPDATE users SET username = 'gestionpro', password = ?, role = 'admin' WHERE id = 'u-admin'", [hashedAdminPw]);
